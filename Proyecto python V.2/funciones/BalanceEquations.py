...
class val():
    def __init__(self,Vol, Ca, Temp):
        self.Vol = Vol
        self.Ca = Ca
        self.Temp = Temp

def PedirDatos(v1, v2, v3):
    v = float(v1)
    c = float(v2)
    t = float(v3)
    return val(v, c, t)
...
def BalanceEquations(v1, v2, v3):
    va1 = PedirDatos(v1, v2, v3)
    V0 = va1.Vol
    Ca0 = va1.Ca
    T0 = va1.Temp
    y0 = [V0,Ca0,T0]
...
master.title("BalanceEquations")

imagenAnchuraMaxima=300
imagenAlturaMaxima=200
# abrimos una imagen
img = Image.open('/home/raul/Documentos/Otros/Proyecto/imagenes/BalanceEquations.png')
...
Label(master, background="SkyBlue3", text="Volumen").grid(row=1)
Label(master, background="SkyBlue3", text="Concentration").grid(row=2)
Label(master, background="SkyBlue3", text="Temperature").grid(row=3)
v1 = StringVar()
v2 = StringVar()
v3 = StringVar()
...
Label(master, background="SkyBlue3", font=(None, 15), text="Balance Equations").place(relx=0.5, rely=0.03, anchor=CENTER)
Button(master, background="DodgerBlue2", text="Calculate"  , command=lambda: BalanceEquations(v1.get(), v2.get(), v3.get())).grid(row=4)
mainloop()
