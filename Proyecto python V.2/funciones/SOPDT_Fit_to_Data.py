...
#Quitas las siguientes 2 lineas

class SOPDT_Fit_to_Data:
    def __init__(self):
        # initial guesses
        p0 = np.zeros(4)
        p0[0] = 3 # Kp
...
# abrimos una imagen
img = Image.open('/home/raul/Documentos/Otros/Proyecto/imagenes/SOPDT_to_Data.png')
...
master.title("SOPDT Fit to Data")
...
Label(master, background="SkyBlue3", text="kp (3)").grid(row=1)
Label(master, background="SkyBlue3", text="taup (5)").grid(row=2)
Label(master, background="SkyBlue3", text="zeta (1)").grid(row=3)
Label(master, background="SkyBlue3", text="thetap (2)").grid(row=4)
v1 = StringVar()
v2 = StringVar()
v3 = StringVar()
v4 = StringVar()
...
Label(master, background="SkyBlue3", font=(None, 15), text="SOPDT Fit to Data").place(relx=0.5, rely=0.03, anchor=CENTER)
Button(master, background="DodgerBlue2", text="Calculate"  , command=lambda: SOPDT_Fit_to_Data()).grid(row=6)
