...
Label(master, text="steps").grid(row=2)
Label(master, text="start time").grid(row=3)
Label(master, text="final time").grid(row=4)
Label(master, text="order (n)").grid(row=5)
Label(master, text="gain (kp)").grid(row=6)
Label(master, text="time constant (taup)").grid(row=7)
v1 = StringVar()
v2 = StringVar()
v3 = StringVar()
v4 = StringVar()
...
Button(master, text="Calculate", command=lambda: Generate_Simulated_Data_from_Model(v1.get(), v2.get(), v3.get(), v4.get(), v5.get(), v6.get())).grid(row=8)
master.mainloop()
